...
<tr><th class="tno">No.</th>
    ...
    <td class="left"><a href="${pageContext.request.contextPath }/notice/count?bno=${list.notice_num }">${list.subject }</a></td>
    <td>${list.name }</td>
...
<c:if test="${vo.startPage > vo.pageBlock }">
	<a href="${pageContext.request.contextPath }/notice/search?pageNum=${vo.startPage - vo.pageBlock}&search=${search}">Prev</a>
</c:if>

<c:forEach var="i" begin="${vo.startPage }" end="${vo.endPage }" step="1">
	<a href="${pageContext.request.contextPath }/notice/search?pageNum=${i }&search=${search}">${i }</a>
</c:forEach>

<c:if test="${vo.endPage < vo.pageCount }">
	<a href="${pageContext.request.contextPath }/notice/search?pageNum=${vo.startPage + vo.pageBlock}&search=${search}">Next</a>
</c:if>
